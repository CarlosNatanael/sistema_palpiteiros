...
def renderizar_pagina_palpites(conn):
    ...
    lista_palpiteiros = [j['nome'] for j in jogadores_db]
...
def premiar_craque():
    ...
    if ja_premiado and ja_premiado['valor'] == '1':
        flash(f'ATENÇÃO! O Craque da Rodada {rodada} já receberam o bônus anteriormente!', 'warning')
        return redirect(url_for('admin_dashboard'))
    ...
    if len(craques) > 1:
        flash(f'🏆 Bônus Adicionado! Os Craques da Rodada: {nomes_craques} fizeram {maior_pontuacao} pts!', 'success')
    else:
        flash(f'🏆 Bônus Adicionado! O Craque da Rodada: {nomes_craques} fez {maior_pontuacao} pts!', 'success')
