...
def init_api_db():
    """Cria e popula o banco de dados da API, se necessário."""
    conn = sqlite3.connect(DB_NAME)
    cursor = conn.cursor()
    cursor.execute('''
    ...
    # Popula com os jogos apenas se a tabela estiver vazia
    cursor.execute("SELECT COUNT(id) FROM jogos")
    if cursor.fetchone()[0] == 0:
        print("Populando o banco de dados da API...")
