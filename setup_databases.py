...
def setup_api_database():
    ...
    print(f"--- Configurando Banco de Dados da API ({API_DB}) ---")
...
def setup_app_database():
    ...
    print(f"--- Configurando Banco de Dados da Aplicação ({APP_DB}) ---")
    conn = sqlite3.connect(APP_DB)
    cursor = conn.cursor()
    cursor.execute('''CREATE TABLE IF NOT EXISTS jogos (id INTEGER PRIMARY KEY, placar_time1 INTEGER, placar_time2 INTEGER, status TEXT, time_que_avancou TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS pontuacao (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT UNIQUE, pontos INTEGER, acertos INTEGER, erros INTEGER, pontos_bonus INTEGER)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS palpites (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, rodada INTEGER, game_id INTEGER, time1 TEXT, time2 TEXT, gol_time1 INTEGER, gol_time2 INTEGER, resultado TEXT, status TEXT, quem_avanca TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS palpite_campeao (id INTEGER PRIMARY KEY AUTOINCREMENT, nome TEXT, campeonato TEXT, time_campeao TEXT, time_campeao_img TEXT, data_palpite TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS campeao_real (id INTEGER PRIMARY KEY AUTOINCREMENT, campeonato TEXT UNIQUE, time_campeao TEXT, time_campeao_img TEXT, data_definicao TEXT)''')
    cursor.execute('''CREATE TABLE IF NOT EXISTS campeao_palpiteiros (id INTEGER PRIMARY KEY AUTOINCREMENT, temporada TEXT UNIQUE, competicao TEXT, nome TEXT, pontos INTEGER, acertos INTEGER, erros INTEGER, data_definicao TEXT)''')
    conn.commit()
    conn.close()
...
if __name__ == '__main__':
    ...
    # Apaga os bancos de dados antigos, se existirem, para garantir uma instalação limpa
    if os.path.exists(API_DB):
        os.remove(API_DB)
        print(f"Banco de dados antigo '{API_DB}' removido.")
    if os.path.exists(APP_DB):
        os.remove(APP_DB)
        print(f"Banco de dados antigo '{APP_DB}' removido.")
